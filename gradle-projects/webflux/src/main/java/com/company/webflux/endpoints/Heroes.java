...
public abstract class Heroes {
    ...
    public static Mono<Squad> getSquad() {
        return client.get()
            .uri("http://localhost/php/util/HTTP/Requests/GET/request.php?file=squad.json")
            .accept(MediaType.APPLICATION_JSON)
            .retrieve()
            .onStatus(HttpStatus::is5xxServerError, clientResponse -> Mono.error(new HttpServerErrorException(clientResponse.statusCode(), "Un error 5xx")))
            .onStatus(HttpStatus::is4xxClientError, clientResponse -> Mono.error(new Exception(clientResponse.statusCode().toString())))
            .bodyToMono(Squad.class);
    }
